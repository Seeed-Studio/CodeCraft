...
cd ${script_dir}/blocks && npm run build-win64
...
cd ${script_dir}/gui && npm install -y --unsafe-perm=true
cd ${script_dir}/gui && npm run build-win64
cd ${script_dir}/gui && rm -rf ../main/app/gui && cp -r ./build ../main/app/gui

echo "======build app====="
cd ${script_dir}/main/app && npm install -y --unsafe-perm=true
cd ${script_dir}/main/app/node_modules/serialport/ && ../.bin/electron-rebuild serialport
