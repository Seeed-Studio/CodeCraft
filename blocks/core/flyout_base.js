...
Blockly.Flyout.prototype.createDom = function(tagName) {
  ...
  */
  ...
  this.svgGroup_.style.borderRight = '1px solid #B2E0FF';
  // this.svgGroup_.style.borderLeft = '1px solid #B2E0FF';
  // this.svgGroup_.style.paddingRight = '5px';
  this.svgGroup_.style.background = 'rgb(228, 244, 255)';
  this.svgGroup_.style.paddingRight = '1px';
...
Blockly.Flyout.prototype.updateDisplay_ = function() {
  ...
  // 因为display为none时会影响删除位置判断，使用z-index来控制积木区的显示应仓
  // this.svgGroup_.style.display = show ? 'block' : 'none';
  this.svgGroup_.style.zIndex = show ? 20 : -20;
...
Blockly.Flyout.prototype.hide = function() {
  ...
  // flyout 关闭时，清除block的阴影效果
  var topBlocks = this.workspace_.getTopBlocks(false);
  for (var i = 0, block; block = topBlocks[i]; i++) {
    block.removeSelectRemoveFilter();
  }

  this.targetWorkspace_.toolbox_.noResultDiv.style.display = 'none';
    ...
    if (xml.tagName) {
      ...
      if (tagName == 'BLOCK') {
        ...
        var recycled = this.recycleBlocks_.findIndex(function(block) {
          return block.id === id && block.disabled === disabled;
        });
