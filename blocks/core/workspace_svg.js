 ...
 * 积木区宽度
 */
Blockly.WorkspaceSvg.prototype.layoutWidth = 250;
...
Blockly.WorkspaceSvg.prototype.translate = function (x, y) {
  if (this.useWorkspaceDragSurface_ && this.isDragSurfaceActive_) {
    ...
    var translation = 'translate(' + (x + 6) + ',' + y + ') ' +
      'scale(' + this.scale + ')';
  ...
  if (this.blockDragSurface_) {
    this.blockDragSurface_.translateAndScaleGroup((x + 6), y, this.scale);
  }
};
...
Blockly.WorkspaceSvg.prototype.showContextMenu_ = function (e) {
  ...
  var snapshootOption = {
    text: Blockly.Msg.SNAPSHOOT,
    enabled: topBlocks.length > 0,
    callback: function () {
      if(ws.snapshoot){
         ws.snapshoot(ws.svgGroup_);
      }
    }
  }

  menuOptions.push(snapshootOption);
...
Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function () {
  ...
  if (this.toolbox_) {
    if (this.toolboxPosition == Blockly.TOOLBOX_AT_TOP ||
      ...
      svgSize.width -= (toolboxDimensions.width - this.layoutWidth); // toolbox宽度减少
    }
  }
  ...
  if (this.toolbox_ && this.toolboxPosition == Blockly.TOOLBOX_AT_LEFT) {
    absoluteLeft = (toolboxDimensions.width - this.layoutWidth); // toolbox宽度减少
  }
  var absoluteTop = 0;
  ...
  var metrics = {
    ...
    toolboxWidth: toolboxDimensions.width - this.layoutWidth,  // toolbox宽度减少
...
Blockly.WorkspaceSvg.prototype.cancelCurrentGesture = function () {
  if (this.currentGesture_) {
    ...
    // 移除block块的时候关闭垃圾容器
    Blockly.utils.removeClass(/** @type {!Element} */(this.toolbox_.categoryMenu_.trashContainer),
      'trashContainerShow');
  }
};
