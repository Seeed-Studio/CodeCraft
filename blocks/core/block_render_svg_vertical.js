 ...
 * @const
 */
Blockly.BlockSvg.ICON_SEPARATOR_HEIGHT = 8 * Blockly.BlockSvg.GRID_UNIT;

/**
 * Path of the top hat's curve.
 * @const
 */
Blockly.BlockSvg.START_HAT_PATH = 'c 25,-22 60,-22 85,0';
...
Blockly.BlockSvg.prototype.updateColour = function () {
  ...
  // this.svgShadowPath_.setAttribute('stroke', strokeColour);
  ...
  this.svgShadowPath_.setAttribute('fill', strokeColour);


  // Render opacity
  this.svgPath_.setAttribute('fill-opacity', this.getOpacity());
  this.svgShadowPath_.setAttribute('fill-opacity', this.getOpacity());

  // Update colours of input shapes.
  for (var i = 0, input; input = this.inputList[i]; i++) {
    if (input.outlinePath) {
      input.outlinePath.setAttribute('fill', strokeColour);
    }
  }
...
Blockly.BlockSvg.prototype.renderDraw_ = function (iconWidth, inputRows) {
  ...
  this.svgShadowPath_.setAttribute('d', pathString);

  // block块中的元素不要有阴影效果
  if (this.svgPath_.parentNode.classList.value.indexOf('blocklyDraggable') > -1) {
    this.svgShadowPath_.setAttribute('transform', 'translate(2,2)');
  }
...
Blockly.BlockSvg.drawStatementInputTop_ = function (steps, cursorX) {
  ...
  steps.push('H', cursorX + Blockly.BlockSvg.STATEMENT_INPUT_INNER_SPACE +
    Blockly.BlockSvg.CORNER_RADIUS - Blockly.BlockSvg.NOTCH_START_PADDING);
  // steps.push('H', cursorX + Blockly.BlockSvg.STATEMENT_INPUT_INNER_SPACE +
  //   2 * Blockly.BlockSvg.CORNER_RADIUS);
  steps.push(Blockly.BlockSvg.NOTCH_PATH_RIGHT);
  steps.push('h', '-' + (Blockly.BlockSvg.STATEMENT_INPUT_INNER_SPACE - Blockly.BlockSvg.GRID_UNIT));
...
Blockly.BlockSvg.prototype.renderDefineBlock_ = function (steps, inputRows,
  ...
  steps.push('v', (row.height - Blockly.BlockSvg.GRID_UNIT));
