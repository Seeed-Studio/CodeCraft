...
Blockly.Block.prototype.getTopBlock = function () {
  var lastBlock = this.getParent();
  if (!lastBlock) return null;
  while (true) {
    var parentBlock = lastBlock.getParent();
    if (!parentBlock) return lastBlock;
    lastBlock = parentBlock;
  }
}

Blockly.Block.prototype.topIsEvent = function () {
  var topBlock = this.getTopBlock();
  if (!topBlock) return false;
  return topBlock.category_ === "events";
}
