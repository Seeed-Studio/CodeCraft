...
// grovezero 
const TwinButton = require('../io/grovezero/twinButton');
const FourDigitDisplay = require('../io/grovezero/fourDigitDisplay');
const Imu9Dof = require('../io/grovezero/imu9Dof');
const Touchpad = require('../io/grovezero/touchpad');
const GestureSensor = require('../io/grovezero/gestureSensor');
const MechKey = require('../io/grovezero/mechKey');
const Knob = require('../io/grovezero/knob');
const PirMotionSensor = require('../io/grovezero/pirMotionSensor');
const Mainboard = require('../io/grovezero/mainboard');
...
class Runtime extends EventEmitter {
    constructor() {
        ...
        this.ioDevices = {
            ...
            // grovezero 
            twinButton: new TwinButton(this),
            fourDigitDisplay: new FourDigitDisplay(this),
            imu9Dof: new Imu9Dof(this),
            touchpad: new Touchpad(this),
            gestureSensor: new GestureSensor(this),
            mechKey: new MechKey(this),
            knob: new Knob(this),
            pirMotionSensor: new PirMotionSensor(this),
    ...
    _registerExtensionPrimitives(extensionInfo) {
        const categoryInfo = {
            ...
            color1: extensionInfo.colour || '#FD7D70',
            color2: extensionInfo.colourSecondary || '#CE645D',
            color3: extensionInfo.colourTertiary || '#CE645D',
    ...
    /**
     * Unregister the primitives provided by an extension.
     * @param {ExtensionMetadata} extensionInfo - information about the extension (id, blocks, etc.)
     * @private
     */
    _unregisterExtensionPrimitives(id) {
        this._blockInfo = this._blockInfo.filter(item => item.id != id);
    }
    ...
    _buildMenuForScratchBlocks(menuName, menuItems, categoryInfo) {
        ...
        return {
            json: {
                ...
                extensions: categoryInfo.extensions || ['scratch_extension'],
    ...
    _convertForScratchBlocks(blockInfo, categoryInfo) {
        ...
        if (blockInfo.blockType == BlockType.BUTTON) {
            blockXML = `<button callbackKey="${blockInfo.opcode}" text="${blockInfo.text}"/>`
        } else {
            blockXML = `<block type="${extendedOpcode}">${context.inputList.join('')}</block>`;
        }
        return {
    ...
    /**
     * Rewrite the getBlocksXML function to match extensions through tags
     * @param {*} tags 
    ...
    getBlocksXML(tags = []) {
        const xmlParts = [];
        for (const categoryInfo of this._blockInfo) {
            const { id, name, color1, color2 } = categoryInfo;
            // Mismatch, continue
            ...
            let refreshable = `refreshable="${new Date().getTime()}"`

            xmlParts.push(`<category name="${name}" id="${categoryInfo.id}"
                ${statusButtonXML} ${colorXML} ${menuIconXML} ${refreshable}>`);
    ...
    attachMeteostationMode(meteostationMode) {
        this.meteostationMode = meteostationMode;
    }

    attachModelsControl(modelsCt) {
        this.modelsControl = modelsCt;
    }

    attachCreateSkillMode(createSkillMode) {
        this.createSkillMode = createSkillMode;
    }
    
    attachCalibrateServosMode(calibrateServosMode) {
        this.calibrateServosMode = calibrateServosMode;
    }
    /**
     * Attach the renderer
    ...
    /**
     * Attach the Abilities module
     * @param {*} storage The Abilities module to attach
     */
        ...
        for (let i = 0; i < this.threads.length; i++) {
            ...
            if (this.threads[i].topBlock === topBlockId && this.threads[i].status !== Thread.STATUS_DONE) {
                ...
                if (this.getIsEdgeActivatedHat(opcode) && this.threads[i].stackClick !== opts.stackClick) {
                    ...
                    const opcodeFunction = this.getOpcodeFunction(opcode);
                    if (opcodeFunction()) {
                        continue;
                    }
                }
    ...
    dispose() {
        ...
        if (this.trainMode) this.trainMode.clear();
        if (this.modelsControl) this.modelsControl.dispose();
     ...
     * @param {*} state 
     */
    updateProjectLoadedState(state) {
        this._projectLoading = state;
    }
     ...
     */
    getMenusForModels() {
        const imageModels = this.modelsControl.imageModels;
        const objectModels = this.modelsControl.objectModels;
        const models = [].concat(imageModels).concat(objectModels);
        const menus = {};
        for (let index = 0; index < models.length; index++) {
            ...
            menus[`modelExtension_menu_blockMenu${m.id}`] = menu;
        }
        return menus;
    }
