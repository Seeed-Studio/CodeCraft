...
const Scratch3MarkBlocks = require('../extensions/scratch3_mark');
const Scratch3WirelessBlocks = require('../extensions/scratch3_wireless');
const Scratch3ThreeAxisAccelerometerBlocks = require('../extensions/scratch3_threeAxisAccelerometer');
const Scratch3MeteostationBlocks = require('../extensions/scratch3_meteostation');
...
const builtinExtensions = {
    ...
    teachableMachine: Scratch3TeachableMachineBlocks,
    cognitiveServices: Scratch3CognitiveServicesBlocks,
    ottoDiyRobot: Scratch3OttoDiyRobotBlocks,
    shieldBot: Scratch3ShieldBotBlocks,
    mark: Scratch3MarkBlocks,
    wireless: Scratch3WirelessBlocks,
    modelExtension: Scratch3ModelExtensionBlocks,
...
class ExtensionManager {
    ...
    clearExtensionLoaded() {
        ...
        // 清除画笔
        if (this.penExtension) {
            this.penExtension.clear();
        }
    }

    /**
     * Remove an extension by URL or internal extension ID
     * @param {*} extensionURL 
     */
    removeExtensionURL(extensionURL) {
        if (this.isExtensionLoaded(extensionURL)) {
            this.pendingExtensions = this.pendingExtensions.filter(item => item.extensionURL != extensionURL)
        ...
        dispatch.call('runtime', '_unregisterExtensionPrimitives', extensionURL).catch(e => {
            log.error(`Failed to unregister primitives for extension on service ${serviceName}:`, e);
        });
    ...
    loadExtensionURL(extensionURL) {
        if (builtinExtensions.hasOwnProperty(extensionURL)) {
            ...
            if (extensionURL === 'pen') {
                this.penExtension = extensionInstance;
            }
