...
import Tooltip from '../tooltip/coming-soon.jsx';
...
const getTooltip = ({ tooltip, overlayStyle, title, children, parentId }) => {
  if (tooltip) {
    const props = tooltip === true ? { overlayStyle, title } : { ...tooltip, overlayStyle, title };
    return <Tooltip place="top" tooltipId={`${parentId}-tooltipId`}  {...props}>{children}</Tooltip>;
  }
  return children;
};
...
export default class Ellipsis extends Component {
  ...
  computeLine() {
    ...
    if (lines && !isSupportLineClamp) {
      ...
      let lineHeight = parseInt(getComputedStyle(this.root).lineHeight, 10);
      if (!lineHeight) {
        lineHeight = 30
      }
  ...
  render() {
    ...
    const isEllipsis = targetCount > 0 && targetCount < text.length;
    const childNode = (
    ...
    return (
      <div {...restProps} ref={this.handleRoot} className={cls}>
        <div ref={this.handleContent}>
          {getTooltip({
            tooltip: isEllipsis ? tooltip : false,
            overlayStyle: TooltipOverlayStyle,
            title: text,
            children: childNode,
            parentId
