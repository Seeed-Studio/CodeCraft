...
const ELFBOT_FIRMWARE_CONFIG_FILE = 'elfbot_firmware_config';

const ESPTOOL_PATH = path.join($dirname, `../../compilers/${system}/esptool/`);
const BIN_PATH = path.join($dirname, '../../buildCache/elfbot/');

const SRC_BIN_FILEPATH = path.join($dirname, '../../static/firmware/elfbot/elfbot.bin');
const DST_BIN_FILEPATH = path.join($dirname, '../../buildCache/elfbot/elfbot.bin');
...
class ElfbotCompiler {
    ...
    saveBin(data) {
        return new Promise((resolve, reject) => {
            let filename = `elfbot_${data.version}.bin`
            netDownload.download(data.url, filename, BIN_PATH, {
                onSucc(localpath) {
                    binName = localpath.substring(localpath.lastIndexOf('elfbot'));
                    resolve()
                    set(ELFBOT_FIRMWARE_CONFIG_FILE, Object.assign({}, data, { binName }));
                },
                onProgress(percent) {
                    console.log(percent);
                },
    ...
    upgrade() {
        ...
        let cmd = (platform === 'darwin' || platform === 'linux') ?
            `cd ${ESPTOOL_PATH} && chmod +x esptool && ./esptool --chip esp32 --port ${this.comName} write_flash -z 0x1000 ${binPath}`
            : `cd ${ESPTOOL_PATH} && .\\esptool.exe --chip esp32 --port ${this.comName} write_flash -z 0x1000 ${binPath}`;
        console.log(cmd);
        return new Promise((resolve, reject) => {
    ...
    async upgrade_local() {
        ...
        let getlatestfirmware = () => {
            return new Promise(resolve => {
                get(ELFBOT_FIRMWARE_CONFIG_FILE, (err, data) => {
                    if (err) {
                        resolve(null)
                    } else {
                        resolve(data)
        ...
        if (latestfirmware &&
            ...
            let cmd = (platform === 'darwin' || platform === 'linux') ?
                `cd ${ESPTOOL_PATH} && chmod +x esptool && ./esptool --chip esp32 --port ${this.comName} write_flash -z 0x1000 ${DST_BIN_FILEPATH}`
                : `cd ${ESPTOOL_PATH} && .\\esptool.exe --chip esp32 --port ${this.comName} write_flash -z 0x1000 ${DST_BIN_FILEPATH}`;
            console.log(cmd);
